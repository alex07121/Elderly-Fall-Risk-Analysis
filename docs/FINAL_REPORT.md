...
## 4. Environment Setup & Step-by-Step Run Instructions
...
```bash
git clone <repository-url>        # see Appendix C
...
## 5. API Endpoints & Schema Documentation
...
| POST | `/predict` | Predict risk for one patient + explanation + suggestions | JSON: `PatientData` (5.2) | `{ id, fall_risk_level, lime_explanations[], suggestion, risk_drivers[] }` |
...
### 5.2 `PatientData` request schema (14 model inputs + 1 optional)
...
### 5.3 `POST /predict` response example
...
`data/fall_risk_patients_2000_v2.csv` — 2,000 elderly-care resident records, 17 columns (14 model features + `patient_id` + `fall_risk_score` + label `fall_risk_level`). Class distribution is imbalanced: MEDIUM 1,133 / HIGH 523 / LOW 344 — the reason `class_weight='balanced'` is mandatory (without it, HIGH recall drops to ~87% per the team's imbalance experiments).

**Preprocessing:** `sex` F→0/M→1; boolean flags → 0/1; `days_since_last_fall` blank → −1 sentinel ("never fell"). No scaling is required for Logistic Regression in this feature space.

### 6.2 Model selection

The team compared **Logistic Regression vs XGBoost vs Random Forest** across two feature sets (17 vs 14 features, "fair comparison" experiments in `ml/experiments/`). **Logistic Regression with the 14-feature set** won on accuracy *and* interpretability — a decisive factor for a clinical tool where every prediction must be explainable.
...
### 7.1 5-Fold Cross-Validation (stability)
...
### 7.2 Hold-out test set (n = 200, threshold = 0.50)
...
Validation accuracy 0.9275 vs test accuracy 0.9150 (gap 1.25%); HIGH recall gap 0.96% — **no overfitting detected**.

### 7.4 Why HIGH recall matters clinically
...
│   └── experiments/       # k-fold CV, SHAP, class balance, final_model.py (saved/ producer)
...
<!-- ============================================================ -->
<!-- ADDENDUM — Phase 1 & Phase 2 Engineering Journey (Kero)      -->
<!-- Partner content above is preserved verbatim. Additions only. -->
<!-- ============================================================ -->

# Addendum — The Engineering Journey: Phase 1 → Phase 2

*This addendum documents the full model-engineering process — every stage's problem, what we changed and why, and how the final system was reached. It complements Sections 6–7 with the process narrative the examiners asked us to show.*

---

## 8.5 Project Background — why this project exists

*Based on the final presentation's opening slides (project goals, global fall statistics, stakeholders, working plan).*

### 8.5.1 The problem, in numbers (global data)
...
### 8.5.2 The three project goals (as presented)
...
### 8.5.3 Stakeholders & their needs
...
### 8.5.4 The working plan (5 stages, as presented)

1. **Collect Data** — create CSV file (2,000 synthetic records, evidence-based features)
2. **Data Cleaning** — handle missing, duplicate, and invalid data
3. **Train Model** — train, test, and evaluate the model (fair protocol, Section 10)
4. **Build Backend** — FastAPI backend, load model, create prediction API
5. **Develop App** — web application UI, connect to API (Vue 3 dashboard)

### 8.5.5 Clinical grounding of the key features
...
## 9. Phase 1 — First Working Pipeline (Problems & Solutions)

### 9.1 Stage: Data Generation (Jim)
...
### 9.2 Stage: First Model (Kero) — the "shortage" we had to fix
...
## 10. Phase 2 — Iterative Improvement Process (Mid-Term → Final)

### 10.1 Iteration 1 — Rigorous validation framework (Kero)
...
### 10.2 Iteration 2 — Feature engineering (Kero)
...
| **Lesson** | Engineered features helped, but we later proved (Section 10.4) they were **redundant** with Alex's feature set — more features ≠ better. |

### 10.3 Iteration 3 — Fair comparison protocol (Kero + Alex)
...
### 10.4 Iteration 4 — Final model = best of both (Kero + Alex)
...
### 10.5 Model selection evidence (why LR, not XGB/RF)
...
LR wins on **both** feature sets and on both metrics — and it is the most interpretable model for a clinical tool. XGBoost's accuracy advantage observed in Phase 1 did not transfer to the v2 data.

### 10.6 Overfitting proof (triple evidence)

| Evidence | Number | Meaning |
|---|---|---|
| Validation vs Test gap | acc 1.25% · recall 0.96% (**< 3% rule**) | Model generalises to unseen data |
...
### 10.7 The thinking process — how we DISCOVERED that more features ≠ better
...
This is a reasonable ML instinct — engineered features *often* help. So we built the 4 interaction features (Iteration 2, Section 10.2) and the 17-feature Kero set looked strong on its own: 5-fold HIGH recall 0.9427, better than the 10-feature Phase 1 baseline.
...
We built `fair_compare.py` (Section 10.3). Once the protocol was locked, the answer became unambiguous:
...
This single insight drove the final decision (Section 10.4): use Alex's minimal 14-feature set, which is both the best performer *and* the most parsimonious — the ideal property for a clinical tool where every feature must be interpretable and every prediction explainable.

---

## 11. Streamlit Dashboard — Explaining the 4-Step Model Evolution

To make the engineering process visible (not just the final result), Kero built a **Streamlit dashboard** (`dashboard.py`) whose **Model Evolution tab walks through the exact 4-step process** of Sections 9–10 with live charts — the same story told in this report:
...
| **Step 2 — Combo Attempt** | Combo (18 features) vs Alex (14) grouped bar chart | Combo is **worse** — Alex 91.7%/99.4% → Combo 91.2%/98.7%, 1 HIGH patient missed → multicollinearity = noise |
| **Step 3 — Fair Comparison** | Kero vs Alex on the same test set | Alex 14 wins (91.5% / 100%, 0 missed vs 1) |
...
## 12. Feature Balance & Clinical Consistency
...
### 12.2 Feature means by risk level (all clinically sensible)
...
Every feature moves in the direction clinical literature predicts — the model learned real clinical patterns, which is why SHAP top-5 stays identical across folds.

---

## Appendix D — Data & Evaluation Integrity Notes (addendum)

- **`fall_risk_score` is never used as a model feature** — it is the label-generation score (target leakage). The model learns purely from the 14 clinical inputs.
- **Threshold 0.50** was selected on the **validation set only**; the test set was touched exactly once, at final evaluation.
- All numbers in Sections 7, 10 and 11 are reproducible with `random_state=42` (see `ml/experiments/`).
